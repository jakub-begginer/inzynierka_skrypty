...
def generate_attack_log():
    ...
    log_entry = {
        ...
        "response": {
            ...
            "body_bytes": random.randint(100, 2048)
        },
        "user_agent": {
            "original": user_agent
        }
...
def save_logs(filename, num_logs, interval):
    while num_logs > 0:
        ...
        with open(os.path.join(log_folder, filename), 'a') as file:
            file.write(str(log_entry) + '\n')  # Zapisz log jako ciąg znaków JSON
        num_logs -= 1
        time.sleep(interval)
...
def main():
    ...
    parser.add_argument('--output-file', type=str, default='access_log.json', help='Output file name')
